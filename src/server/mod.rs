...
use crate::{server::internal::{request, SERVER_IP, get_order_from_db}, send_and_receive_data};
                ...
                match color {
                    ...
                    0 => {
                        if objects[Color::Red as usize] > 0 {
                            ...
                            positions.push(Position { position_x:x, position_y:y });
                        }
                    }
                    1 => {
...
pub fn send_order_done_db(positions:Vec<Position>, order_id: u32){
    ...
    for p in positions{
        ...
        order_send.push(OrderSend{ position_x, position_y, product_type_id});
    }
    let data_to_send = json!({"updated_positions": order_send});

    let full_request = format!("REPORT orders/{} ORDSYS/1.0\nStatus: OK\n{}", order_id, data_to_send);
    ...
    if order_json == None{
        ...
        for line in order_json.lines(){
            ...
            if line.contains("\"product_type\""){
                ...
                let total_product_str = line[product_total_start + "\"total_product_amount\": ".len()..product_total_end].trim();
