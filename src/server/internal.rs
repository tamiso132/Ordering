...
pub(crate) mod request{
    ...
    pub(crate) const REQUEST_GET_ORDER_POSITIONS_FULL: &'static str = "GET order-positions ORDSYS/1.0";
...
pub(crate) fn update_position(x:u32, y:u32, color:Color){
    ...
    let mut temp = vec![];
        temp.push(TempPos{position_x: x as u32, position_y: y as u32, product_type_id: color as u32});
...
pub(crate) fn get_positions_from_db() -> [[i8;4];6]{
    let order_json = send_and_receive_data(SERVER_IP, request::REQUEST_GET_ORDER_POSITIONS_FULL).unwrap();
    ...
    let mut grid_info:[[i8;4];6] = [[0;4];6];
